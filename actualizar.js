...
const ruta = path.join(__dirname, 'controllers', 'pagosController.js');

let contenidoOriginal = '';
if (fs.existsSync(ruta)) {
    contenidoOriginal = fs.readFileSync(ruta, 'utf8');
}

// Extraemos las funciones nativas viejas si existieran para no borrarlas
let cuerpoLimpio = contenidoOriginal.replace('const pagosController = {', '').replace(/module\.exports\s*=\s*pagosController\s*;?\s*$/, '');

// Si el archivo estaba vacío, inyectamos las firmas estructurales por seguridad
if(!cuerpoLimpio.includes('mostrarFormulario')) {
    cuerpoLimpio = `
    mostrarFormulario: async (req, res) => { try { res.render('pagos/registrar'); } catch(e) { res.redirect('/'); } },
    mostrarFormularioGrupal: async (req, res) => { try { res.render('pagos/registrar_grupal'); } catch(e) { res.redirect('/'); } },
    guardar: async (req, res) => { try { res.redirect('/prestamos'); } catch(e) { res.redirect('/'); } },
`;
}

// Limpiamos llaves huérfanas al inicio si quedaron del replace
cuerpoLimpio = cuerpoLimpio.trim().replace(/^\{/, '');

const codigoUnificado = `const db = require('../config/db');
...
fs.writeFileSync(ruta, codigoUnificado, 'utf8');
console.log('\n==================================================================');
console.log('✅ ¡Controlador Pagos actualizado e inyectado con éxito total!');
console.log('==================================================================\n');
